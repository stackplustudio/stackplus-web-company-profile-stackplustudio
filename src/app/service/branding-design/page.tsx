...
// brandingPricingData dihapus dari import karena kita pakai database sekarang
import { brandingPricingTabs, expertiseData, brandingLifecycleData } from "@/data";
...
export default function BrandingPricingPage() {
  ...
  // Filter agar "Branding Design" tidak muncul di kotak Explore Other Services
  const otherServices = expertiseData.filter(
    (item) => item.title !== "Branding Design"
  );
    ...
    const fetchPricing = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select("*")
          .eq("service_type", "BRANDING") // Pastikan Tipe Layanan sesuai
    ...
    <main className="min-h-screen pt-10">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-16">
          ...
          <h2 className="text-5xl md:text-6xl font-bold text-[#0053f1] tracking-tight mb-12">Branding</h2>
              ...
              >
                ...
                <div className="mb-12">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="w-10 h-10 bg-[#290000] border border-[#FF7C00]/30 rounded-xl flex items-center justify-center shadow-md">
                       <span className="text-[#FF7C00] font-bold text-lg font-sans leading-none tracking-tighter">Ai</span>
                    </div>
                  </div>
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 border-t border-gray-200">
        ...
        <div className="flex flex-col relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-x-12 lg:gap-y-6">
            {brandingLifecycleData.slice(0, 4).map((item, index) => (
          ...
          {/* BARIS 2: Sisa 2 Item */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-x-12 lg:gap-y-6">
            {brandingLifecycleData.slice(4, 6).map((item, index) => (
              <div key={index} className="relative h-full flex flex-col">
                <div className="h-full flex flex-col bg-[#FDFBF7] border border-[#EAE5D9] p-6 lg:p-8 rounded-[1.5rem] shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group z-10 relative">
                  <div className="bg-[#423E3A] text-white text-[10px] sm:text-xs font-bold px-2 py-3.5 rounded-full text-center w-full shadow-md mb-6 tracking-wide group-hover:bg-primary transition-colors">{item.title}</div>
                  <div className="w-full h-px bg-[#EAE5D9] mb-6"></div>
            ...
            {/* Sisa 2 kolom dikosongkan agar sejajar dari kiri */}
