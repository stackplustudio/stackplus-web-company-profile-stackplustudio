...
// aiChatbotPricingData dihapus dari import karena kita pakai database sekarang
import { aiChatbotPricingTabs, expertiseData, aiChatbotLifecycleData } from "@/data";
...
// --- TAMBAHAN FUNGSI WHATSAPP SAJA ---
const getWhatsAppLink = (planName: string) => {
  const phoneNumber = "6281398410264"; 
  const message = `Halo StackPlus Studio! Saya ingin diskusi mengenai layanan *AI Chatbot* untuk paket *${planName}*. Bisa bantu informasinya?`;
    ...
    const fetchPricing = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select("*")
          .eq("service_type", "AI CHATBOT") // Pastikan Tipe Layanan sesuai
    ...
    <main className="min-h-screen pt-10">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-16">
          ...
          <h2 className="text-5xl md:text-6xl font-bold text-[#0053f1] tracking-tight mb-12">AI Chatbot</h2>
        ...
        {/* Grid 4 Kolom Pricing dengan Efek Hover */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mt-12">
          
          {/* Logika Loading & Empty State (Desain tidak disentuh, hanya bungkusannya) */}
          {isLoading ? (
            ...
            pricingData.map((plan, index) => (
              <div 
                ...
                className={cn(
                  ...
                  plan.is_pro // Di database namanya is_pro (bukan isPro)
              ...
              >
                ...
                {/* Fitur Code Type */}
                {plan.code_types && plan.code_types.length > 0 && (
                  <>
                ...
                <div className={cn("w-full border-t border-dashed mb-8", plan.is_pro ? "border-white/30" : "border-gray-200", (!plan.code_types || plan.code_types.length === 0) && "mt-auto")}></div>
                ...
                {/* HANYA INI YANG DIUBAH: DARI <button> JADI <a> KE WHATSAPP */}
                <a 
      ...
      {/* SECTION 3: LIFECYCLE AI CHATBOT */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 border-t border-gray-200">
        ...
        <div className="flex flex-col relative">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-x-12 lg:gap-y-6">
            {aiChatbotLifecycleData.slice(0, 4).map((item, index) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-x-12 lg:gap-y-6">
            {aiChatbotLifecycleData.slice(4, 7).map((item, index) => (
              <div key={index} className="relative h-full flex flex-col">
                <div className="h-full flex flex-col bg-[#FDFBF7] border border-[#EAE5D9] p-6 lg:p-8 rounded-[1.5rem] shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group z-10 relative">
                  <div className="bg-[#423E3A] text-white text-[10px] sm:text-xs font-bold px-2 py-3.5 rounded-full text-center w-full shadow-md mb-6 tracking-wide group-hover:bg-primary transition-colors">{item.title}</div>
                  <div className="w-full h-px bg-[#EAE5D9] mb-6"></div>
